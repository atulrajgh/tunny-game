...
- **All in-game messages live in ONE placeholder `.message-bar`** (amber bar, flex, wrapped) rendered where the old `turn-indicator` sat (between the table and the viewer's hand) and on the review screen. Segments concatenate inside it: connection-loss, error/status toast text, redeal (reason + count + **Redeal** button), timeout (**Take Over** button), the current-action line (who is bidding/selecting trump/whose turn, folded in from the removed `.current-action`), and **Your turn!**. The bar renders `null` when no message applies. While an error/status toast is showing (e.g. **Invalid play**), the bar's background turns red (`.message-bar.error`); it reverts to amber when the 5s toast clears. The standalone fixed `.reconnect-banner`, `.toast.error`, `.redeal-banner`, `.timeout-banner`, `.waiting-banner`, and `.turn-indicator` elements are no longer used on the game/review screens (login screen keeps its own `.toast.error`/`.reconnect-banner`). Old banner CSS classes are retained for other/legacy styling; `UIViewer.js` previews the consolidated bar and the 6-column review table.
- **Start Game** (`start_game` → `startCut()`) and **Determine Dealer** (`cut_done` → `determineDealer()`) also live in the message bar, not the admin panel: during `waiting` the bar shows a **Start Game** button (disabled until 4 seats are filled and **at least one human is seated** — `startCut` also refuses a bot-only table) and during `cut` a **Determine Dealer** button. While short on humans (`humanSeated < 4`) with bot slots free (`botCount < 3`), the waiting button becomes **Add Bots** instead — same `add_bot` event as the panel button (adds a spectator bot the admin then seats to a position). All four flow buttons (**Start Game**, **Add Bots**, **Determine Dealer**, **Redeal**) are clickable by **any seated human player or the host admin** and render disabled (with a "(seated players only)" hint) for observers/bots — `canStartFlow` = viewer is not a bot and (holds a position or is the admin). The server gates `start_game`/`cut_done`/`redeal`/`add_bot`-via-message-bar identically (`g.players.includes(v) || v.isAdmin`, `!v.isBot`; the admin panel Add Bot button remains admin-only). `determineDealer()` is idempotent (`if (this.state !== 'cut') return false;` — the server `cut_done` handler also checks state first), so two seated players double-clicking can never re-deal the cut.
- **Assign Seats** (`assign_seats` → `gc.assignSeats(adminId)`) sits in the waiting message bar **before** the Start Game/Add Bots group and is **admin-only** (disabled with a "(admin only)" hint for everyone else). It auto-arranges the table: unseated **humans** are seated at random open positions, then remaining open positions go to **existing unseated spectator bots** (`promoteSpectator`) — it **never adds new bots** and never displaces a seated player. Only when the gallery has **no unseated humans** does the host admin take a seat (`adminSit`), so a solo host can sit and the table always keeps ≥1 human (which unlocks Start Game). Server responds with `seats_assigned` (`{ summary: 'Name@Pos, …' }`, new seats only). It's a no-op (→ `error('Nothing to assign')`) outside `waiting` or when every seat is filled; the frontend disables the button in those cases (`seatedCount < 4 && (unseatedHumans > 0 || unseatedBots > 0 || adminUnseated)`).
