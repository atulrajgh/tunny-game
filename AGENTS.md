...
- **Start Game** (`start_game` → `startCut()`) and **Determine Dealer** (`cut_done` → `determineDealer()`) also live in the message bar, not the admin panel: during `waiting` the bar shows a **Start Game** button (disabled until 4 seats are filled) and during `cut` a **Determine Dealer** button. All three flow buttons (**Start Game**, **Determine Dealer**, **Redeal**) are clickable by **any seated human player or the host admin** and render disabled (with a "(seated players only)" hint) for observers/bots — `canStartFlow` = viewer is not a bot and (holds a position or is the admin). The server gates `start_game`/`cut_done`/`redeal` identically (`g.players.includes(v) || v.isAdmin`, `!v.isBot`). `determineDealer()` is idempotent (`if (this.state !== 'cut') return false;` — the server `cut_done` handler also checks state first), so two seated players double-clicking can never re-deal the cut.
